...
class BudgetSafetyTest {
    ...
    void healthyClusterDeliversNearlyEverything() {
        ...
        // The gap from 1.0 is the last partial top-up on each shard: a wallet holding less
        // than the cheapest request cannot spend its remainder.
        assertThat(cluster.deliveredFraction()).isGreaterThan(0.99);
    }
    ...
    void repeatedCrashesNeverCauseOverspend() {
        ...
        final BudgetCluster cluster =
                new BudgetCluster(sim, BudgetCluster.Config.defaults(), NetworkConditions.lan());
        cluster.start();
    ...
    void clockSkewNeverCausesOverspend() {
        final BudgetCluster.Config config = BudgetCluster.Config.defaults();
    ...
    void chaosNeverCausesOverspend() {
        ...
        for (long seed = 1; seed <= CHAOS_SEEDS; seed++) {
            ...
            assertThat(cluster.authority().totalGrantedMicros())
    ...
    void crashesStrandBudget() {
        ...
        final BudgetCluster cluster = new BudgetCluster(
                sim, BudgetCluster.Config.defaults().withTopUpMicros(500_000L), NetworkConditions.lan());
        cluster.start();
        ...
        assertThat(cluster.authority().strandedByRestartsMicros()).isPositive();
        assertThat(cluster.overspendMicros()).isZero();
    ...
    private static BudgetCluster run(long seed, NetworkConditions conditions) {
        ...
        final BudgetCluster cluster = new BudgetCluster(sim, BudgetCluster.Config.defaults(), conditions);
        cluster.start();
        sim.runUntil(RUN_LENGTH);
        return cluster;
    }
